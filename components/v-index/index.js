Component({
  ...
  methods: {
    _updateDataChange() {
      const indexItems = this.getRelationNodes('../v-index-item/index');
    ...
    setTouchStartVal() {
      wx.createSelectorQuery().in(this).select('.v-index_fixed').boundingClientRect(res => {
        this.setData({
          startTop: res.top
        })
      }).exec()
